...
冻结任务：准备一个已有失败测试的半成品 Python 待办 CLI。优先级需求和部分测试已经存在，但实现尚未完成；MiniCoder 需要根据真实失败完成 high/medium/low 优先级、默认值、排序和旧数据兼容，并按需要补充边界测试。

视频需要清楚出现：

1. 展示目标项目初始测试真实失败，并输入修复任务。
2. Agent 浏览和读取项目及失败测试。
3. Agent 根据错误定位未完成实现。
4. Agent 精确修改多个文件并补充必要测试。
5. Agent 再次执行测试；若仍失败，继续根据真实错误修复。
6. 全部测试通过后，展示最终 diff 和实际命令运行结果。

失败来自半成品项目的真实未完成实现，不伪造终端输出，也不在录制时手工替 Agent 修改代码。
